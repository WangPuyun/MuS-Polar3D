...
parser.add_argument("--model_name", type=str, default=None,
                    help="Load pre-trained model to continue training. Default=None (train from scratch), set to '/**.pth' to resume training")
...
parser.add_argument('--checkpoints_dir', default="./pt/AttentionU2Net/",
                    help='Path to model checkpoint files (for resuming training)')
...
def main_worker(local_rank, nprocs, args):
    ...
    for epoch in epoch_iter:
        ...
        model, train_loss_list = config.train_sfp(train_loader, model, criterion, optimizer, epoch, writer,
                                                  args.local_rank, args, train_loss_list)
        val_loss_list = config.val_sfp(val_loader, model, writer, epoch, args.local_rank, args, criterion,
                                       val_loss_list)
        ...
        if args.local_rank == 0:
            ...
            if val_loss_list[-1] < min_val_loss:
                ...
                best_model_path = './pt/AttentionU2Net_best'
                # Find all .pth files
